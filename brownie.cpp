...
//						int ntax=taxa->GetNumTaxonLabels();
						...
						for (int chosentreenum=0; chosentreenum<inObservedTrees.GetNumTrees(); chosentreenum++) { //loop over all the observed gene trees
							...
							if (useperl) {
							   ...
							   perlscript+="		}";
							   perlscript+="\n";
							   perlscript+="		elsif ($ARGV[$i] =~ /-d(\\d+)/) {";
							   perlscript+="\n";
							   perlscript+="			$desiredtree=$1;";
							   perlscript+="\n";
							   perlscript+="		}";
							   perlscript+="\n";
							   perlscript+="	}";
							   ...
							   perlscript+="\n";
							   perlscript+="			if ($desiredtree==$obstreecount || $desiredtree==0) {";
							   perlscript+="\n";
							   ...
							   perlscript+="				}";
							   perlscript+="\n";
							   perlscript+="				if ($obstreecount>1 && $desiredtree==0) {";
							   perlscript+="\n";
							   perlscript+="					print \"\\n\";";
							   perlscript+="\n";
							   perlscript+="				}";
							   ...
							   perlscript+="}";								
								...
								finalsystemcall+=" -d";
								finalsystemcall+=chosentreenum+1;
								finalsystemcall+=" > ";
								finalsystemcall+=msinputfile;
							}
							else {
		...
		if (currentnode!=(*Tptr).GetRoot() ) {
			...
			for (int breaknum=breaksperbranch;breaknum>=0;breaknum--) { //Break nums are numbered from the tip down, we want to go from the bottom up
				...
				if (j>(discretecharacters->GetObsNumStates(discretechosenchar))) {
					...
					errormsg+=" currentnode = ";
					errormsg+=static_cast<ostringstream*>( &(ostringstream() << currentnode) )->str();
					errormsg+=" breaknum = ";
					errormsg+=breaknum;
					errormsg+=" i = ";
					errormsg+=i;
				...
				if (debugmode) {
					cout<<endl<<"currentnode "<<currentnode;
