...
Brownie uses the 'ratetest' command to perform this test allowing the user to specify a number of optional parameters and so function RBrownie provides functions which add this command along with various options to the \texttt{commands} slot of a \texttt{brownie} object.  It requires the use of a \texttt{phylo4d\_ext} object as input (see the manual) - Let's look at an example:
...
<<ContOne,echo=TRUE,eval=TRUE,keep.source=TRUE>>=
require(RBrownie)
data(parrot)
...
<<ContTwo,echo=TRUE,eval=FALSE,keep.source=TRUE>>=
# run the brownie objects as is and get the text 
# output from the brownie core
rawtxt = run.asis(par.new)$textout 
rawtxt = scan.textout(rawtxt)  # process text output 
...
In the example above \texttt{runNonCensored} removes all commands currently in the \texttt{commands} slot and replaces them with the same commands we added earlier manually and then executes the resulting \texttt{brownie} list (all within the function).  In returns the same data.frame.  
...
<<DiscreteOne,echo=TRUE,eval=TRUE,keep.source=TRUE>>=
require(RBrownie)
data(geospiza_ext)
...
At this point we have essentially re-created the geospiza\_ext object. Now we will manually run the analysis:
<<DiscreteTwo,echo=TRUE,eval=FALSE,keep.source=TRUE>>=
rawout = run.asis(geoext)
@
If we look at the \texttt{rawout} we will see that both list elements \texttt{textout} and \texttt{treesout} are non-null.  The \texttt{treesout} element includes the trees with reconstructed discrete values mapped on to either branches - in this case two model,freqs combinations were analyzed so two tree reconstructions are returned.  But first, process the text output using some familiar commands.

<<DiscreteThree,echo=TRUE,eval=FALSE,keep.source=TRUE>>=
...
<<DiscreteFour,echo=TRUE,eval=FALSE,keep.source=TRUE>>=
...
tree1 = read.simmap(text=treesout[1]) 
tree2 = read.simmap(text=treesout[2]) 
#
...
As shown above all of the \texttt{runTEST} commands can also be put together manually using \texttt{addTEST} and \texttt{addOPTION} commands.  There is an add command for most Brownie options and commands - to view the available options, see:
...
We are continually updating \verb@RBrownie@ and would love your feedback on how it can be improved. If there is a feature you would like to see supported in either Brownie or RBrownie, please post a feature request to the \href{http://groups.google.com/group/brownie-users}{brownie-users google group}.
