...
par.new <- addNonCensored(par.new,treeloop=T,charloop=T,taxset="all",filereplace=T)
par.new <- addModel(par.new,"BMS")
par.new <- addNonCensored(par.new,treeloop=T,charloop=T,taxset="all",fileappend=T,filereplace=F)
commands(par.new)
@
...
test.results = runNonCensored(parrot,models=brownie.models.continuous()[1:2],treeloop=T,charloop=T)
summaryCont(test.results)
@

In the example above \texttt{runNonCensored} removes all commands currently in the \texttt{commands} slot and replaces them with the same commands we added earlier manually and executes the \texttt{brownie} list.
