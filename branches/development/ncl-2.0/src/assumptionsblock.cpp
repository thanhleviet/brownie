...
void AssumptionsBlock::HandleTaxset( NexusToken& token )
{
    ...
    token.GetNextToken();
    if (token.Equals("?") ) { //BCO added this section for taxset help
        cout<<"Usage: Taxset <taxon-set-name> = <taxon-list>"<<endl<<endl<<"The complement to the taxset is automatically created,\nand has the prefix NOT."<<endl;
        token.GetNextToken();
         //Clunky.
    } //BCO
    if( token.Equals("*") ) {
   ...
   // Now grab the equals sign
    ...
    /**Section added to this function by BCO to automatically create a complement to the defined taxset, which has the same name but is preceded by NOT.
        */
    nxsstring complement_taxset_name;
    complement_taxset_name="NOT";
    complement_taxset_name+=taxset_name;
    IntSet currenttaxsetlist;
    currenttaxsetlist = GetTaxSet( taxset_name );
    IntSet complementtaxonnumbers;
    for (int i=0;i<totalTaxa;i++) {
        if(currenttaxsetlist.count(i)==0) {
            complementtaxonnumbers.insert(i);
        }
    }
