	...
	: taxa(tb), assumptionsBlock(ab), NexusBlock()
{
	...
	symbols[1] = '1';
	symbols[2] = '\0';
        values = new float[NCL_MAX_VALUES]; //BCO.
        nvalues=0; //BCO This goes up by one every time we look at taxon, character pair
...
//BCO added below
/**
* @method GetValue [char:public]
 * @param i [int] the taxon in range [0..ntax)
* j [int] the character in range [0..nchar)
*
* Returns value from values list representing the state for taxon i
* and character j.  
* Assumes values is not equal to NULL and the taxon is not polymorphic.
*/
float CharactersBlock::GetValue( int i, int j, bool incorrectj)
{
    if (incorrectj) {
	j = charPos[j];
    }
    int valuenumber;
    valuenumber=(i*nchar)+j; //we want the number of the cell, where cells
    //                         are numbered from 0 to (nchar*ntax)-1. Every
    //                         row we go down adds nchar to this number, then
    //                         we move over j characters.
    float currentvalue;
    currentvalue=values[valuenumber];
    return currentvalue;
}
//BCO added above
...
void CharactersBlock::HandleFormat( NexusToken& token )
{
	...
	for(;;)
	{
		...
		else if( token.Equals("ITEMS") )
		{
			...
			token.GetNextToken();
			//BCO  if( !token.Equals("STATES") ) {
				//BCO  errormsg = "Sorry, only ITEMS=STATES supported at this time";
				//BCO  throw XNexus( errormsg, token.GetFilePosition(), token.GetFileLine(), token.GetFileColumn() );
			//BCO  }
...
bool CharactersBlock::HandleNextState( NexusToken& token, int i, int j )
{
   ...
   if( p != equates.end() ) {
	...
	else
	{
		...
		if( !uncertainty && !polymorphism && datatype !=continuous) { // BCO datatype != put in by O'Meara
			int k = HandleTokenState( token, j );
			matrix->AddState( i, j, k );
		}
                if( !uncertainty && !polymorphism && datatype == continuous) {//BCO added this loop
                    if (token.Equals("-")) { //then we have a negative number, but have only recovered the hyphen
                        negvalue=true;
                        token.GetNextToken();
                    }
                    else {
                    ...
                    HandleTokenValue( token, nvalues);
                    //token.SetLabileFlagBit( NexusToken::hyphenNotPunctuation );
                    matrix->AddState( i, j, nvalues);
                    nvalues++;
                }
		else {
         int tildeFound = 0;
...
//BCO added the function below this
/**
* @method HandleTokenValue [int:protected]
 * @param token [NexusToken&] the token used to read from in
 * @param j [int] the number of the value we're on.
* 
*/


int CharactersBlock::HandleTokenValue( NexusToken& token, int j )
{
    if (j > NCL_MAX_VALUES) {
        errormsg="The size of your character matrix is greater than the size allowed by this\ncompilation of Brownie. Sorry! To fix this, go to the source code and change\nNCL_MAX_VALUES in the nexusdefs.h file to a larger number (at least the number\nof taxa times the number of characters in your matrix) or email Brian at\nbcomeara@ucdavis.edu for a new compilation of Brownie.\n\nCongrats on having so much data, by the way.";
        throw XNexus( errormsg, token.GetFilePosition(), token.GetFileLine(), token.GetFileColumn() );
    }
    nxsstring currenttoken;
    currenttoken=token.GetToken();
    float t = atof( currenttoken.c_str() );
    if (negvalue) {
        values[j]=-1*t;
    }
    else {
    ...
    return 0;
    //BCO note that we don't look up the state's position. We assume that each state is unique, and just add it to the appropriate place in the values array.
}
//BCO added the function above this
...
void CharactersBlock::HandleMatrix( NexusToken& token )
{
	...
	//BCOif( datatype == continuous ) {
	//BCO	errormsg = "Sorry, continuous character matrices have not yet been implemented";
	//BCO	throw XNexus( errormsg, token.GetFilePosition(), token.GetFileLine(), token.GetFileColumn() );
	//BCO }
...
void CharactersBlock::Report( std::ostream& out )
{
	...
	if( gap != '\0' )
        ...
        if (datatype != continuous) { //BCO put the following loop contents into this new if loop.
   ...
   if( tokens )
   {
      ...
      if( n == 0 && matrix->IsGap( i, j ) )
         ...
         out << missing;
          ...
          if (datatype != continuous) { //BCO added loop
...
LabelListBag::const_iterator ci = charStates.find(j);
         ...
         // been eliminated
            ...
            // show label at index number s in LabelList at ci
              ...
              float v=CharactersBlock::GetValue( i , j,false);
              out << v;
          } //BCO
      }
      else {
