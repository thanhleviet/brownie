...
// $Id: gtree.cpp,v 1.1.1.1 2006/05/21 15:37:59 bcomeara Exp $
...
void GNode::Calc (GPlotInfo &plot)
{
	...
	{
#if USE_WXWINDOWS
		xy.y = (int)(plot.r.GetTop() + plot.count * plot.leafgap);
		plot.lasty = xy.y;
#else
		...
		if ((plot.style & TS_PHYLOGRAM) == TS_PHYLOGRAM)
		{
			...
			{
#if USE_WXWINDOWS
				xy.x = (int)(plot.r.GetLeft() + ((PathLength / plot.maxheight) * plot.r.GetWidth()));
#else
			...
			{
#if USE_WXWINDOWS
				xy.x = (int)(plot.r.GetRight() + ((PathLength / plot.maxheight) * plot.r.GetWidth()));
#else
	...
	else // ------- internal node
	{
		...
		if ((plot.style & TS_CLADOGRAM) == TS_CLADOGRAM)
		{
			...
			{
#if USE_WXWINDOWS
				xy.x = (int)(plot.r.GetLeft() + plot.nodegap * (plot.leaves - Weight));
#else
			...
			{
#if USE_WXWINDOWS
				xy.x = (int)(plot.r.GetRight() - plot.nodegap * (plot.leaves - Weight));
#else
			...
			{			
#if USE_WXWINDOWS
				xy.y = (int)(plot.lasty - (((Weight - 1) * plot.leafgap) / 2));
#else
